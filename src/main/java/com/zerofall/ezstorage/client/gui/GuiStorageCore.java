...
public class GuiStorageCore extends GuiContainer {
    ...
    protected static int autoRefillAmount = 64;
    ...
    private static final int BTN_AUTO_REFILL_AMOUNT = 5;
    ...
    private final SideButton[] sideButtons = {
            ...
            new SideButton(BTN_AUTO_REFILL_AMOUNT)
    };
    ...
    private boolean shiftQuickMoveActive = false;
    private final Set<Integer> shiftQuickMovedSlots = new HashSet<Integer>();
    ...
    private String getLabelFor(int btnId)
    {
        switch (btnId)
        {
            ...
            case BTN_AUTO_REFILL:
                return "A";
            case BTN_AUTO_REFILL_AMOUNT:
                return String.valueOf(autoRefillAmount);
            default:
                return "?";
        }
    }
    ...
    private String getTooltipFor(int btnId)
    {
        switch (btnId)
        {
            ...
            case BTN_AUTO_REFILL_AMOUNT:
                return StatCollector.translateToLocalFormatted("hud.msg.ezstorage.autorefill.amount", autoRefillAmount);
            default:
                return "";
        }
    }
    ...
    private void handleButtonClick(SideButton btn, int mouseButton)
    {
        switch (btn.id)
        {
            case BTN_SORT_MODE:
            ...
            case BTN_AUTO_REFILL_AMOUNT:
                int step = GuiScreen.isShiftKeyDown() ? 16 : 1;

                if (mouseButton == 1)
                {
                    autoRefillAmount -= step;
                }
                else
                {
                    autoRefillAmount += step;
                ...
                if (autoRefillAmount < 1)
                {
                    autoRefillAmount = 1;
                }
                else if (autoRefillAmount > 64)
                {
                    autoRefillAmount = 64;
                }
    ...
    {
        handleShiftQuickMoveGesture(mouseX, mouseY);
    ...
    protected void renderMouseOverTooltip(int mouseX, int mouseY)
    {
        ...
        {
            this.drawItemStackTooltip(mouseOverItem, mouseX, mouseY);
        }
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton)
    {
        ...
        {
            Slot playerSlot = getPlayerInventorySlotAt(mouseX, mouseY);

            if (playerSlot != null)
            {
                this.shiftQuickMoveActive = true;
                this.shiftQuickMovedSlots.clear();
                quickMoveSinglePlayerSlot(playerSlot);
                return;
            }
        }
        ...
        if (mouseButton == 0 || mouseButton == 1)
        {
            for (SideButton btn : sideButtons)
            {
                if (btn.contains(mouseX, mouseY))
                {
                    handleButtonClick(btn, mouseButton);
                    return;
                }
            }
        ...
        if (slot != null && (mouseButton == 0 || mouseButton == 1))
        {
            ...
            EmiNetwork.sendToServer(new C2SInvSlotClickedPacket(index, mouseButton, mode));
    ...
    private void handleShiftQuickMoveGesture(int mouseX, int mouseY)
    {
        if (!this.shiftQuickMoveActive)
        {
            return;
        }
        ...
        {
            this.shiftQuickMoveActive = false;
            this.shiftQuickMovedSlots.clear();
            return;
        }

        Slot slot = getPlayerInventorySlotAt(mouseX, mouseY);

        if (slot != null)
        {
            quickMoveSinglePlayerSlot(slot);
        }
    }

    private Slot getPlayerInventorySlotAt(int mouseX, int mouseY)
    {
        ...
        for (Object object : this.inventorySlots.inventorySlots)
        {
            ...
            int sx = this.guiLeft + slot.xDisplayPosition;
            int sy = this.guiTop + slot.yDisplayPosition;
    ...
    private void quickMoveSinglePlayerSlot(Slot clickedSlot)
    {
        ...
        if (!this.shiftQuickMovedSlots.add(clickedSlot.slotNumber))
        {
            return;
        }

        int windowId = this.inventorySlots.windowId;
        this.mc.playerController.windowClick(windowId, clickedSlot.slotNumber, 0, 1, this.mc.thePlayer);
    }
    ...
    private void applyVisualSlotClick(int slotIndex, int mouseButton, int mode)
    {
        if (mode == 1)
        {
            // Shift-transfer affects player inventory slots; avoid speculative cursor changes.
    ...
    public void updateScreen()
    {
        ...
        {
            if (inventoryUpdateTimestamp != container.inventoryUpdateTimestamp || needFullUpdate)
            {
    ...
    private static void loadSettingsFromConfig() {
        ...
        autoRefillAmount = EZConfiguration.guiAutoRefillAmount.getIntegerValue();
        if (autoRefillAmount < 1) autoRefillAmount = 1;
        if (autoRefillAmount > 64) autoRefillAmount = 64;
        if (saveSearch) {
    ...
    private static void saveSettingsToConfig() {
        ...
        EZConfiguration.guiAutoRefillAmount.setIntegerValue(autoRefillAmount);

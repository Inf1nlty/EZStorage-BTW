...
public class TileEntityInventoryProxy extends TileEntity implements ISidedInventory {
    ...
    private void resolveCoreIfNeeded() {
        if (this.core != null || this.worldObj == null) {
            return;
        }
        ...
        if (block instanceof StorageMultiblock multiblock) {
            this.core = multiblock.findCore(new BlockRef(block, this.xCoord, this.yCoord, this.zCoord), this.worldObj, null);
        }
    }
    ...
    @Override public String getInvName() {
        return "container.ezstorage.input_port";
    }

...
public class EZInventoryManager {
    ...
    /**
     * Registers an already-loaded inventory with a fixed ID and persists it immediately.
     */
    public static EZInventory registerInventory(String id, EZInventory inventory)
    {
        if (inventory == null)
        {
            return null;
    ...
    private static void saveToFile(NBTTagCompound tag, File file)
    {
        ...
        catch (IOException ioException)
        {
            ...
            EZStorageAddon.instance.LOG.warn("Couldn't write inventory to file system.", ioException);
        }
    }
    ...
    private static NBTTagCompound readFromFile(File file) {
        ...
        if (file.exists())
        {
            ...
            {
                EZStorageAddon.instance.LOG.warn("Couldn't read inventory file. Try falling back to backup, if exists.", ioException);
                ioException.printStackTrace();
            }
        }
        ...
        if (tag == null && fileOld.exists())
        {
            ...
            {
                EZStorageAddon.instance.LOG.warn("Couldn't read inventory backup file.", ioException);
                ioException.printStackTrace();
            }
        }
        ...
        if (server == null)
        {
            ...
            for (WorldServer worldServer : server.worldServers)
            {
                ...
                for (Object object : worldServer.playerEntities)
                {
                    ...
                    {
                        if (container.inventory == inventory && player instanceof EntityPlayerMP)
                        {
                            EmiNetwork.sendToClient((EntityPlayerMP) player, new S2CStoragePacket(inventory));
                        }
                    }
                }
